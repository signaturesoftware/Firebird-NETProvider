...
internal sealed class GdsConnection
{
	...
	public GdsConnection(string dataSource, int port, int timeout)
		: this(null, null, dataSource, port, timeout, 8192, Charset.DefaultCharset, 3, false, Version13.WireCryptOption.Enabled, null, null, AuthPluginOption.Dynamic)
	{ }

	public GdsConnection(string user, string password, string dataSource, int portNumber, int timeout, int packetSize, Charset charset, short dialect, bool compression, Version13.WireCryptOption wireCrypt, byte[] cryptKey, string impersonateUser, AuthPluginOption authPlugin)
	{
		User = user;
		Password = password;
