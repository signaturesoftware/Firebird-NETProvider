...
sealed class AuthBlock
{
	...
	SrpClient _srp;
	SspiHelper _sspi;
	ImpersonateAuthClient _impersonateAuth;

	public GdsConnection Connection { get; }
	public string User { get; }
	public string Password { get; }
	public WireCryptOption WireCrypt { get; }
	public AuthPluginOption AuthPlugin { get; private set; }
	...
	public byte[] UserIdentificationData()
	{
		...
		{
			if (AuthPlugin != AuthPluginOption.Impersonate_Auth)
			{
			...
			if (!string.IsNullOrEmpty(User))
			{
				...
				var pluginNameBytes = AuthPlugin switch
				{
					AuthPluginOption.Impersonate_Auth => Encoding.UTF8.GetBytes(_impersonateAuth.Name),
					_ => Encoding.UTF8.GetBytes(_srp256.Name),
				};
				...
				var specificData = AuthPlugin switch
				{
					AuthPluginOption.Srp => Encoding.UTF8.GetBytes(_srp256.PublicKeyHex),
					AuthPluginOption.Impersonate_Auth => Encoding.UTF8.GetBytes(_impersonateAuth.GetEncryptedPassowrd(Password)),
	...
	public void Start(byte[] serverData, string acceptPluginName, bool isAuthenticated, byte[] serverKeys)
	{
		...
		else if (AcceptPluginName.Equals(_impersonateAuth.Name, StringComparison.Ordinal))
		{
			if (hasServerData)
			{
