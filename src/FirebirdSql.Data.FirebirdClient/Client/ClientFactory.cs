...
internal static class ClientFactory
{
	...
	private static DatabaseBase CreateManagedDatabase(ConnectionString options)
	{
		...
		var connection = new GdsConnection(options.UserID, options.Password, options.DataSource, options.Port, options.ConnectionTimeout, options.PacketSize, charset, options.Dialect, options.Compression, FbWireCryptToWireCryptOption(options.WireCrypt), options.CryptKey, options.ImpersonateUser, FbAuthPluginToAuthPluginOption(options.AuthPlugin));
		connection.Connect();
		try
		{
